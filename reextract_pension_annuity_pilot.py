# -*- coding: utf-8 -*-
"""報告57 任務C 第7組聚合題材（勞工退休金雙制度授權鏈+提繳比例對照）
Stage 0 targeted重抽。

框架：N0030022_勞工退休金條例年金保險實施辦法（子法）§1「依勞工退休金條例
（以下簡稱本條例）第三十五條第三項及第三十七條規定訂定之」——又一個雙來源
授權（比照第6組候選的就業促進津貼實施辦法，母法兩條不同條文共同授權同一
子法）。母法§14（個人退休金專戶提繳）與§36（年金保險提繳）皆訂有「不得
低於勞工每月工資百分之六」的最低比例——兩種制度比例其實相同，差別在於
年金保險制度另需事業單位僱用勞工人數達200人以上、經工會或勞資會議同意
（§35）才能採行，是測試系統會不會誤判兩種制度比例不同的天然聚合題材。

已直接讀`original.md`與對應svo-chunk-*.md逐字核對，內容與所引用條號一致。

targeted chunk（不重抽整份文件——子法69個SVO chunk、母法67個，只碰真正
需要的條文）：
  - N0030022 chunk 1（§1法源依據）
  - N0030020 chunk 15/37/40/41（§14個人專戶6%、§35年金保險授權條款、
    §36年金保險6%、§37保險人資格授權）

安全模式（同任務C第2-6組）：不走force_rebuild=True，改用
...
用法：
    ...
    OLLAMA_EMBEDDING_NUM_GPU=0 nohup python -u reextract_pension_annuity_pilot.py \
        >> "<job>/tmp/reextract_pension_annuity_pilot.log" 2>&1 &
完成訊號：PILOT-DONE
"""
...
TARGETS: list[tuple[str, list[int]]] = [
    ("N0030022_勞工退休金條例年金保險實施辦法", [1]),
    ("N0030020_勞工退休金條例", [15, 37, 40, 41]),
]
