...
class TestCase(BaseModel):
    ...
    trap_claim_spans: List[str] = Field(
        ...
        description=(
            ...
            "刻意維持純字串比對（不用LLM judge），這是確定性優先的設計選擇，跟本專案"
            "services/interval_lookup_service.py既有的G2方案E（查表判斷移出LLM "
            "改用確定性Python檢查）原則一致。RefusalBench（Muhamed et al. 2025）"
            "只是間接參考：它量的是模型在脈絡有缺陷時是否適當拒答，不是判斷答案是否"
            "為拒答，準確範圍見報告61 §3.1。**只在陷阱結論能表達成固定字串時"
