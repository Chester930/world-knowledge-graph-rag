...
3. **PDF 圖片區塊回填至全文最後，而非原本視覺所在的段落位置**：DOCX／PPTX 的圖片內容會插在它在
   原文件中實際所在的段落/投影片順序位置（跟周圍文字交錯呈現）；PDF 則是所有頁面本文先輸出完，
   圖片區塊才依頁碼分組、整批附加在全文最後（`--- [第 N 頁圖片解析] ---`）。這是因為要做到「插入
   原始段落位置」，需要把圖片座標整合進雙欄版面重建演算法（`_pdf_track_pdfminer` 的
   span/left_col/right_col 拓撲排序）本身，風險是可能弄壞既有驗證過的雙欄還原邏輯，故選擇「頁面
   分組」的折衷版：圖片內容不會遺漏、順序上仍照頁碼與頁內視覺順序排列，只是位置上不是精確插入
   原始段落位置。
4. **PPTX 投影片若以「設定背景填滿圖片」而非「插入圖片」的方式放置圖片，會被完全跳過**：
   `python-pptx` 的 `slide.shapes` 只列舉一般 shape（含全版面大小的 Picture shape），不包含透過
   `slide.background.fill` 設定的背景填滿圖片。此為範圍很小的邊界案例（正常製作簡報通常是插入
   圖片而非設背景），跟「整份掃描 PDF 頁面即為一張大圖」那種結構性問題不同——PPTX 內嵌圖片（含
   全版面大小的圖片）本身已由既有的逐 shape 迴圈正確處理，此限制僅侷限於背景填滿這個特定路徑。
