...
def test_file_config_source_reads_json(tmp_path):
    ...
    (tmp_path / "kg").mkdir()
    (tmp_path / "domain_packs" / "generic.json").write_text(
    ...
    cfg = ConfigLoader([FileConfigSource(tmp_path)]).load("kg-7")
    assert cfg.bfs.seed_max_degree == 250
    assert cfg.bfs.per_seed_limit == 77
