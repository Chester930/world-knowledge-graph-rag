...
- `claim_next_pending()` 原子認領（挑選 + 標 processing 一步），所以**可以同時
  跑 2-3 個本腳本的實例**做並行抽取，彼此不會撞到同一個 chunk。
...
用法（3 個並行 worker，各自的 log）：
    for i in 1 2 3; do nohup python -u drain_236903cf.py --label w$i \
        >> "<job>/tmp/drain_236903cf_w$i.log" 2>&1 & done
"""
...
from services.task_queue_service import claim_next_pending
...
async def main(label: str) -> None:
    ...
    print(f"{tag}drain 開始：kg={KG_STR}  db={db_path}", flush=True)
    processed = 0
    empties = 0
    t0 = time.monotonic()
    while True:
        ...
        try:
            ...
            print(f"  {tag}!! {source} c{chunk_index} 例外：{e!r}", flush=True)
        processed += 1
        if processed % 20 == 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--label", default="", help="log 前綴，並行多 worker 時用來區分")
