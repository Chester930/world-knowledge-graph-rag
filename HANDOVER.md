...
3. **定向重抽驗證正在背景執行中**（本 session 的背景 fork，尚未回報結果）：對 `18-Q5`／`57-AGGR7`／`57-AGGR8`／`57-AGGR19`／`57-CANARY5` 這5個已知失敗案例對應的 KG#4 chunk 做定向重抽（`scripts/kg/reextract_chunks.py`，非全量），比對重抽前後的 `fact_text`，確認條件是否恢復、有無新增回歸。**這是對 KG#4（`236903cf-055a-40a8-8923-b9d06601f3b7`，正式資料，非沙盒）的即時資料異動**，結果會回填進報告65 §6（尚未寫入）。
4. **待辦順序（明確）**：(a) 等定向重抽驗證 fork 回報結果 → (b) 依結果寫入報告65 §6 並判斷方向A是否有效 → (c) **驗證確認無誤後**才能交付 Codex 開始報告66的 T1-T6。跳過 (a)/(b) 直接做 (c) 會有兩個 agent 同時改 `_svo_prompt()` 的衝突風險。
