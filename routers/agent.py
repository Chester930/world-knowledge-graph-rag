    ...
    *,
    ...
    if has_context:
        ...
        instruction = (
            ...
            # 不准取最近的一列、不准外插到沒檢索到的級距（RefusalBench 有定義
            # GranularityMismatch 類別＝所問與所給的細節層級不一致；「級距缺一段時模型
            # 誤當成可查表而硬答」是本專案金絲雀驗證 P3 觀察到的，不是該論文的結論）。
...
async def _generate_from_context_lines(
    ...
    *,
    ...
    # （直接證據是本專案自己的金絲雀驗證；RefusalBench 僅間接參考——它在單文件
    # NQ 基準上報告 Qwen 各尺寸「脈絡有缺陷時是否適當拒答」<17%，量的是生成行為，
    # 不是查表判斷，見報告61 §3.1）。只在能明確解析出
