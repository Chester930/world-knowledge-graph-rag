...
1. **本報告確認** → 下載 CatRAG 全文（arXiv:2602.01965）進 `docs/參考文獻/04_圖遍歷與大節點問題/`，補該資料夾 README 內容清單與交叉引用
2. **L0 + L1**（純查詢端：`svo_hops` 預設、`chat()` 順序、`bfs_query` 範圍約束、seed degree 上限）——主 drain（B/全量）跑期間即可實作、加測試
