...
### 1.0 研究前置查核結論（2026-09-15原稿；2026-09-16重新查證）

> ⚠️ 原稿本節宣稱「已同步至論文§2.4.9/§3.2§f/§4.7.4/§5.5.3a」不實，已於文首核實標注訂正（見上方2026-09-16複查段落）。以下取代原稿內容，是**真正針對本任務問題重新查證的結果**，詳細機制與誠實侷限見 `docs/參考文獻/33_圖遍歷結果之原始段落回補與雙軌上下文組裝/README.md`，索引也已補進 `docs/論文/文獻與專案查核表.md`。

實際查證（WebFetch對arXiv HTML全文查關鍵章節，非逐頁精讀）找到三篇真正對題的文獻／專案，PDF已下載進repo：

- **Think-on-Graph 2.0**（Ma et al. 2024/2025，**ICLR 2025**，`arXiv:2407.10805`）——**目前找到最直接對應本任務雙軌設計的先例**。論文§3.3「Reasoning with Hybrid Knowledge」原文明確描述送入LLM的prompt同時包含「triple paths」與「corresponding context chunks」，架構跟本任務「軌道A邏輯摘要＋軌道B原始Chunk」幾乎同構。**誠實侷限**：§4.7支持證據是人工案例分類統計（雙重增強答案占32.26%），**不是控制變因的消融實驗**，不能拿來當作本任務K+C消融的預期效果基準。
- **KAG**（Liang et al. 2024/2025，Ant Group，`arXiv:2409.13731`）——摘要明確提到「mutual-indexing between knowledge graphs and original chunks」，佐證「圖與原始chunk應有雙向索引」這個架構需求已在工業界（電子政務/醫療QA）落地。**誠實侷限**：僅查證摘要層級，未精讀方法章節確認雙向索引的實作粒度；2wiki/HotpotQA的F1提升數字是整體框架綜合效果，不能歸因於「回補原始段落」這一個子機制。
- **HippoRAG**（Gutiérrez et al. 2024，NeurIPS，已有PDF）——WebFetch查證§2.3方法機制：PPR節點分數透過`P`矩陣映射回passage分數，**最終送進LLM的是原始段落文本、不是圖結構摘要**，是「圖遍歷後結果映射回原始文本」的既有頂會先例。**但走的是「取代」路線（只留段落）而非本任務設想的「並陳」路線（摘要+段落都留）**，且論文消融分析（Table 5）沒有對比這兩種路線的效果差異。

**修正後的誠實結論**：三篇文獻沒有一篇提供嚴格控制變因的量化消融，證明「加入原始段落文本」相對於「只用結構化三元組摘要」有可重現的效果提升——ToG-2的支持證據是人工案例統計，HippoRAG根本沒做這個對比，KAG只有整體框架分數。**這代表§4驗收標準規劃的「K vs K+C消融實驗」在文獻上找不到現成答案可以直接套用，必須靠本專案自己的消融量出來**——原稿「沒有文獻支持『增加完整Chunk必然修好所有拒答或改寫錯誤』」這句結論性判斷本身仍然成立，只是現在有了真正查證過的文獻基礎，而非空話。
