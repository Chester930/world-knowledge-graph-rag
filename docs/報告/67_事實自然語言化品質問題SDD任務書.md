...
**建立日期**：2026-09-22
**文件性質**：診斷＋任務書。T1 已完成，T2/T3 已實作，T4 尚未執行——本報告是報告65 §10「另開任務處理」的落地；報告65處理的是三元組拆解粒度（抽取端），本報告處理的是**已抽取三元組改寫成自然語句**（`services/svo_service.py::_naturalize_triple()`，報告24機制）本身的生成品質，兩者是不同的函式、不同的問題層次，不要混在一起改。
...
4. 本次 T1 調查沒有修改 KG 資料；後續已依本結論完成 T2/T3 的純程式碼與單元測試變更，T4 backfill 範圍估算仍未執行。

### 1.4 T2/T3 實作結果（2026-09-22）

T1 判定為 LLM／prompt 問題後，已完成 T2 與 T3；本輪沒有執行 T4、沒有重抽任何 KG，也沒有對 Neo4j 寫入。

1. **T2（`services/svo_service.py`）**：新增
   `_naturalization_leaked_type_marker()`，將非空 `subject_type`／`object_type` 以逗號切成 token，逐詞檢查自然語句中是否出現；邊界避免把 `PERSONAL` 誤判成 `PERSON`，並保留「概念」作為合法語意詞的例外。裸字與重複逗號列表命中時，與量詞遺漏核對共用同一個 `_verbalize_fact()` fallback。prompt 也明確禁止照抄型別名稱。
2. **T3（`routers/agent.py`）**：改用 `core/constants.py::ENTITY_TYPES` 建立受控 token 正則，支援裸字與全形括號逗號列表；只清除受控型別，不再以任意大寫英文字作萬用匹配，因此清單外的合法英文縮寫保留。既有 `（概念）` 清除行為維持。
3. **測試**：新增 T2 的裸字、逗號列表、合法語意詞、子字串邊界與 fallback/prompt 測試，以及 T3 的裸字、逗號列表、清單外縮寫測試。指定全套命令結果：**1065 passed, 8 warnings, 75.63s**。
...
- **T3 發現一個真實缺陷並已修復（commit `6bc644e`）**：`_TYPE_MARKER_RE`（`routers/agent.py`）的裸字（無括號）分支**只有結尾邊界、缺開頭邊界**，導致「XORGANIZATION」這類尾端剛好是受控型別詞的合法詞彙被誤砍成「X」（`SUPERPERSON`→`SUPER`）——已用真實案例重現確認，且T3自己的測試（用`NPO`/`ABC`這類完全不在清單裡的縮寫）沒有涵蓋到這個邊界，沒抓到。**已直接修正**：裸字分支補上對稱的 `(?<![A-Za-z0-9_])`，加1項回歸測試，全套pytest **1066 passed**。括號版本不需要這個左邊界（全形括號本身就是天然斷詞）。

### T4 完成（2026-09-22，Claude Code唯讀執行，未對KG寫入）

對 KG#4（`236903cf-055a-40a8-8923-b9d06601f3b7`）全部 11,011 筆帶 `natural_text` 的邊套用 T2 的 `_naturalization_leaked_type_marker()` 核對邏輯：

**受影響：96 筆（0.87%）**。規模不算大，但確實存在，且部分案例相當嚴重（例如 `'本保險效力開始及停止於INSURANCE EFFECTBEGINANDSTOP'` 整句崩壞、`'DISPUTE指勞資雙方當事人基於法令...'` 型別詞取代了主詞本身）。分布上沒有集中在特定文件或關係型別，散布在多份不同法規文件。

**是否要backfill**：0.87%（96筆）規模不大，若要backfill，技術路徑是對這96筆邊重跑 `_naturalize_triple()`（T2修好後應該就不會再洩漏），比照 `backfill_fact_text_embeddings()` 的既有模式寫一支唯讀先篩選、再視使用者決定要不要回填的批次工具——**本次只完成統計，不執行backfill**，需要使用者另外決定。
