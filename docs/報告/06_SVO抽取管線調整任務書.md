...
**目前定案方向**：句子已逐句算好 embedding（`SENTEMBED`，見下方 §3.2），但「怎麼組塊」本身傾向先採**簡單固定聚合**（依句數/字數上限分組，小 chunk 為主，呼應 GraphRAG 附錄的方向性發現），語意斷點式分組（07 報告的核心演算法）因文獻支撐不足暫緩，是否採用留給第五章消融實驗實測比較，而非直接採信文獻宣稱。組塊規則的具體數值（句數上限、字數上限、是否重疊）仍待討論定案。
