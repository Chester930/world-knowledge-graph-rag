...
**目前定案方向**：句子已逐句算好 embedding（`SENTEMBED`，見下方 §3.2），但「怎麼組塊」本身傾向先採**簡單固定聚合**（依句數上限分組，小 chunk 為主，呼應 GraphRAG 附錄的方向性發現；原本另有字數上限，2026-07-22 已拿掉，見下方一則），語意斷點式分組（07 報告的核心演算法）因文獻支撐不足暫緩，是否採用留給第五章消融實驗實測比較，而非直接採信文獻宣稱。
...
- [x] 文件內實體別名登記表（3.4 §a，PK 動態提升機制）——`services/entity_registry_service.py`（新增），含**長度優先＋出現次數次要規則**（`should_promote_by_length()`；2026-07-21 三度修訂後 §a 文件內範圍改採長度優先，頻率優先規則 `should_promote_by_frequency()` 保留在 §b 跨文件範圍——原本此處誤寫「頻率優先＋長度次要」，2026-09-10 Pass 2 L4 審視訂正）、規則式別名比對（子字串/縮寫）、LLM 仲裁 hook、斷點續傳快照，測試見 `tests/services/test_entity_registry_service.py`（22 項）
...
- [x] **系統接線（2026-07-21）**：`routers/staging.py::_trigger_extraction()` 在 `assign()`／`confirm_cluster()`／`classify()`（自動分配）三個端點呼叫 `assign_document_to_kg()` 之後，同步觸發 `CHUNKREADY`（`prepare_svo_ready_chunks()`）＋`ENQUEUE`（`task_queue_service.enqueue()`）；`main.py` 的 `lifespan` 新增呼叫 `svo_service.create_entity_index()`（原本從未被呼叫）與 `task_queue_service.ensure_ready()`（`RESTART` 進入點，`KGRepository` 為 stub 時降級為空 KG 清單、不讓 app 啟動失敗）。測試見 `tests/routers/test_staging.py`（3 項）。
  - ⚠️ **路徑訂正（2026-09-10，Pass 2 L3/L4 審視）**：此抽取觸發函式已於報告 11 P0-2 從 `routers/staging.py::_trigger_extraction()` 遷移至 `services/svo_service.py::trigger_extraction()`（供 `knowledge_graph_service.build_graph()` 共用同一套 CHUNKREADY→EMBEDCHUNK→ENQUEUE 邏輯），並改為明確 `driver` 參數（不再函式內自呼 `get_driver()`）。本清單第 107、108、112 項所述的 `routers/staging.py::_trigger_extraction()` 路徑均以此為準——router 端呼叫點仍在 `routers/staging.py`，只是實作本體已移出至 service 層。`articles=`（法規領域專屬 `ArticleAware` 路徑）自 2026-08-24 起亦由此函式轉交。
