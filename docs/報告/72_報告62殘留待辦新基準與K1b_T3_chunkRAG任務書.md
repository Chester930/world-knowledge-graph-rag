...
## 11. S3 評分器規則 R 敏感度分析（2026-09-25）

### 11.1 範圍、輸入與判定口徑

本節依報告79 T1–T4 做**純離線重算**，沒有重跑評測 harness、沒有呼叫 Neo4j／Ollama，也沒有修改 `services/atomic_scorer.py` 或任何 production 評分邏輯。新腳本為 `scripts/eval/s3_scorer_rule_sensitivity.py`，直接 import 報告62 §14 已有的 `flip()`、`perfect_prime()`、`meets_standard()`、`scope_ok()` 與 `load()`、`overlap()`，沒有複製規則實作。

輸入範圍如下：

- S0：`baseline_runs/20260923_rebased/{stage_s0_r1,stage_s0_r1_resume1,stage_s0_r2}`，共 75 筆有效 records。
- B0：`candidate_runs/{s3_chunk_rag_b0_stage_a,stage_b,stage_c}`，共 69 筆有效 records。
- B1：`candidate_runs/{s3_chunk_rag_b1_stage_a,stage_b}`，共 67 筆有效 records。
- 題目範圍：`baseline_runs/20260923_rebased/frozen_manifest.json` 的 42 筆 `eligible_ids`，不是舊的 `20260920_frozen` manifest；規則 R 閾值 `T=0.7`。
- 本次未啟用舊工具的 `--cand-fail`／`--require-qty` 事後旗標。每題結論沿用既有工具的有效執行嚴格過半多數決；所以 B0 的 `57-AGGR19` `[False, True, True]` 在本節診斷口徑會算通過，與 §9 為保守起見列為 unstable 的 20/42 不同。

完整逐筆原判定、規則 R 判定、stage 與專項證據輸出於 `data/eval/candidate_runs/s3_rule_r_rejudge.json`。

### 11.2 T2：規則 R 前後的臂內結果與配對比較

| 臂 | 有效 records | 原評分器 | 規則 R | R 改為通過 | R 改為未通過 |
| --- | ---: | ---: | ---: | --- | --- |
| S0 K | 75 | 13/42 | 16/42 | `18-Q6`、`57-DIST1`、`57-DIST2`（3） | — |
| B0 naive chunk-RAG | 69 | 21/42* | 23/42 | `57-DIST2`、`57-AGGR7`（2） | — |
| B1 hybrid chunk-RAG | 67 | 21/42 | 22/42 | `57-AGGR18`（1） | — |

「原評分器」欄仍是以本節指定的逐題多數決重算，並未覆寫 §9 的正式／保守摘要。B0 的 `21/42*` 之所以比 §9 的 20/42 多一題，只有 `57-AGGR19` 的 `[False, True, True]` 不穩定口徑差異。

| 配對 | 口徑 | 對照新增（S0 未通過 → 對照通過） | KG 優勢（S0 通過 → 對照未通過） | KG 相對差 | exact McNemar `p` |
| --- | --- | --- | --- | ---: | ---: |
| S0 vs B0 | 原評分器（多數決） | 10 | 2 | **−8** | 0.038574 |
| S0 vs B0 | 規則 R | 10 | 3 | **−7** | 0.092285 |
| S0 vs B1 | 原評分器 | 9 | 1 | **−8** | 0.021484 |
| S0 vs B1 | 規則 R | 8 | 2 | **−6** | 0.109375 |

規則 R 對三個 arm 都適用，沒有把對照臂固定不動。B1 的原始 9 題新增中，`18-Q6`／`57-DIST2` 因 S0 在 R 下翻盤而不再是對照獨有；同時 `57-AGGR18` 在 B1 端被 R 重判通過，且 S0 仍未通過，形成新的對照新增。故不能把 S0 的 +3 題直接解讀成 KG 相對於 chunk-RAG 的 +3 題。

### 11.3 T3：`18-Q6`／`57-DIST2` 專項證據

下列數字由既有 `overlap()` 與 `flip()` 逐筆重算；兩題的 S0 執行結果均一致，`scope_ok=True` 且 `deterministic_guard_passed=True`，因此題目層級多數決都翻成通過。

| 題目 | S0 `missing_spans` | S0 答案中的對應文字 | overlap | 數字／序號全在答案 | 無否定詞 | `flip()` | 題目結論 |
| --- | --- | --- | ---: | --- | --- | --- | --- |
| `18-Q6` | `一年內合計不得超過十四日` | `一年內事假不得累計超過十四日` | **1.000** | 是（`一`、`十四`） | 是 | 是 | 未通過 → 通過 |
| `57-DIST2` | `給予一至三日之特別休假` | 答案明確寫出直轄市／縣市級為「一至三日」的特別休假，使用「可獲得…的」句式 | **0.900** | 是（`一`、`三`） | 是 | 是 | 未通過 → 通過 |

`18-Q6` 的答案把 gold 的「合計」改成「事假…累計」，但法律效果與數值都已表達；`57-DIST2` 則是「給予…之」與「可獲得…的」的表面句式差異。兩題的 overlap、數字存在與無否定詞三條件都通過，直接支持報告78 所指的 scorer surface-form 假陰性假說。不過這是樣本內敏感度分析，不是真值裁判，也沒有把規則 R 升格為正式評分器。

### 11.4 T4：規則 R 下的結論與限制

對較強的 B1 chunk-RAG，原本 §9.4 的 S0 K 13/42 對 B1 21/42、KG **−8** 題，在本節規則 R 多數決下變成 S0 K **16/42** 對 B1 **22/42**、KG **−6** 題；exact McNemar `p` 由 **0.021484** 變為 **0.109375**。因此：

1. `18-Q6`／`57-DIST2` 確實是評分器表面形式造成的部分假陰性，規則 R 讓落差縮小 2 題；原始「B1 顯著領先」的配對統計強度在此敏感度分析下不再穩健。
2. 但規則 R 後 KG 仍低於 B1 6 題，沒有出現 KG 整體反超或整體增益的證據。因此 §9.4 的方向性結論——「目前沒有顯示 KG 相對於做得夠好的 chunk-RAG 有整體增益」——**在方向上仍成立，但原本 −8 題與 `p=0.0215` 的落差幅度／顯著性不能視為對評分器規則不敏感**。更準確的說法是：目前觀察到的劣勢至少有一部分可能是表面 span 假陰性，修正敏感度後仍是 chunk-RAG 領先，但證據強度較弱。
3. B0 的規則 R 多數決結果由 21/42 變為 23/42，S0 由 13/42 變為 16/42，KG 相對差由 −8 變為 −7；方向也沒有翻成 KG 優勢。這個 B0 數字使用本節多數決，不能與 §9 保守列出的 20/42 直接混為同一正式摘要。

本節仍受 §9.5 已列限制約束：`57-AGGR18` 的角色互換歸屬檢查（`role_mismatch` 規則泛化）尚未完成，可能使 Atomic Accuracy／達標 status 高估，且同樣影響 S3 與本次規則 R 解讀；共用 `qwen2.5:7b` generator/judge 仍只是 pilot。規則 R 只保留在離線診斷腳本，不改變任何 production 預設或既有評測結果。

---

## 12. S3 落差題獨立模型盲審交叉驗證（2026-09-26）

### 12.1 方法、範圍與持久化

本節依報告81 T1–T4 做**唯讀盲審分析**，沒有寫入 Neo4j、沒有修改 production 程式碼，也沒有重跑 S3 評測。新腳本 `scripts/eval/s3_scorer_blind_crosscheck.py` 直接 import `scripts/eval/scorer_audit_crosscheck.py` 的 `PROMPT`、`ask()`、`unload()`、`compact_answer()`；兩個模型均沿用既有 `ask()` 的 `think=false`、temperature 0、seed 0 設定。

樣本從 `baseline_runs/20260923_rebased/{stage_s0_r1,stage_s0_r1_resume1,stage_s0_r2}` 的 S0 K arm records 讀取，只納入 `18-Q1`、`18-Q6`、`57-COREF3`、`57-DIST2`、`57-AGGR6`、`57-AGGR14`、`57-AGGR19`，排除 `17-Q6` 與 `canary-P4`。7 題共得到 **11 個 unique `(question_id, missing_span)` 案例**：`57-AGGR6` 有 4 個 span，`57-AGGR19` 有 2 個，其餘各 1 個。每個案例只把題目、完整 S0 答案與 missing span 給盲審模型，不提供原評分器或規則 R 標籤；答案送出前只使用既有 `compact_answer()` 去除重複行。結果逐筆保存於 `data/eval/candidate_runs/s3_blind_crosscheck.json`。

下表的「原評分器」與「規則 R」直接讀取報告79 `s3_rule_r_rejudge.json` 的既有**題目層級**判定，沒有重新計算；同一題的多個 missing span 因此會重複該題判定。`支持`／`不支持` 是獨立模型對該 span 是否被答案明確斷言的盲審結果。

### 12.2 T2：逐一 missing span 的五方比較

| 題目 | Missing span | 原評分器 | 規則 R | granite4.2:3b | qwen3.5:4b |
| --- | --- | --- | --- | --- | --- |
| `18-Q1` | 住院者，二年內合計不得超過一年 | 不支持 | 不支持 | 支持 | 不支持 |
| `18-Q6` | 一年內合計不得超過十四日 | 不支持 | 支持 | 支持 | 支持 |
| `57-COREF3` | 勞工為親自照顧家庭成員，除本法或其他法律另有規定者外，得依前項規定請事假，並得擇定以小時為請假單位 | 不支持 | 不支持 | 支持 | 支持 |
| `57-DIST2` | 給予一至三日之特別休假 | 不支持 | 支持 | 支持 | 不支持 |
| `57-AGGR6` | 但勞工同時符合勞動基準法第五十三條規定時，雇主應依勞動基準法第五十五條及第八十四條之二規定發給勞工退休金。 | 不支持 | 不支持 | 支持 | 不支持 |
| `57-AGGR6` | 雇主依第二十三條第一款、第三款，或勞工依第二十四條第二款至第四款規定終止勞動契約者，雇主應依勞動基準法之規定，發給勞工資遣費。 | 不支持 | 不支持 | 支持 | 不支持 |
| `57-AGGR6` | 雇主依第二十三條第二款，或勞工依第二十四條第一款規定終止勞動契約者，雇主應依勞動基準法之規定，發給勞工退休金。 | 不支持 | 不支持 | 支持 | 不支持 |
| `57-AGGR6` | 雇主依第八十四條第一項第一款、第三款，或勞工依前條第一項第二款至第四款規定終止勞動契約者，雇主應按勞工工作年資，適用勞動基準法或勞工退休金條例規定，發給勞工資遣費。 | 不支持 | 不支持 | 支持 | 不支持 |
| `57-AGGR14` | 從業人員人數逾十人者，應置就業服務專業人員至少三人，並自第十一人起，每逾十人應另增置就業服務專業人員一人。 | 不支持 | 不支持 | 不支持 | 不支持 |
| `57-AGGR19` | 職業災害勞工依第二十四條第一款規定終止勞動契約時，準用勞動基準法規定預告雇主。 | 不支持 | 不支持 | 不支持 | 不支持 |
| `57-AGGR19` | 雇主依第二十三條規定預告終止與職業災害勞工之勞動契約時，準用勞動基準法規定預告勞工。 | 不支持 | 不支持 | 不支持 | 不支持 |

### 12.3 T3：一致率、具體分歧與兩個翻盤案例

11 筆案例全部取得兩個模型的可解析 JSON 回覆，沒有 N/A。與既有判定的一致率如下：

| 盲審模型 | 對原評分器一致 | 對規則 R 一致 |
| --- | ---: | ---: |
| `granite4.2:3b` | **3/11（27.3%）** | **5/11（45.5%）** |
| `qwen3.5:4b` | **9/11（81.8%）** | **9/11（81.8%）** |
| 兩模型彼此一致 | — | **5/11（45.5%）** |

具體分歧顯示模型並未形成穩定的第三方標準：

- `granite4.2:3b` 與原評分器不一致的 8 筆為 `18-Q1`、`18-Q6`、`57-COREF3`、`57-DIST2` 及 `57-AGGR6` 的 4 個 spans；它把這些答案判為支持，但其中多數仍是原評分器與規則 R 都判不支持的案例。它與規則 R 的 6 筆不一致為 `18-Q1`、`57-COREF3` 與 `57-AGGR6` 的 4 個 spans。
- `qwen3.5:4b` 與原評分器只在 `18-Q6`、`57-COREF3` 不一致；與規則 R 不一致則為 `57-COREF3`、`57-DIST2`。因此它對原判定與 R 判定都得到 9/11，不能用一致率本身選出哪個版本是真值。
- 兩模型彼此在 `18-Q1`、`57-DIST2` 與 `57-AGGR6` 的 4 個 spans 分歧，合計 6 筆不一致；其餘 5/11 才一致。特別是 `57-AGGR6` 的四個具體退休金／資遣費分支，granite 全部判支持，qwen 全部判不支持。

報告79 的兩個規則 R 翻盤案例呈現不同結果：

- `18-Q6`：原評分器「不支持」→ R「支持」；兩個盲審模型都判「支持」。這對 R 的翻盤提供一個局部、但不是定論的支持訊號。
- `57-DIST2`：原評分器「不支持」→ R「支持」；granite 判「支持」，qwen 判「不支持」。qwen 的理由是答案雖提到「一至三日」，但把直轄市／縣市級模範警察與好人好事代表的適用對象混在一起；因此這題沒有跨模型的穩定佐證。

### 12.4 T4：誠實解讀與結論

本次盲審的訊號比報告62 §14.6 的經驗更不一致：granite 對既有標籤只有 3/11 或 5/11 一致，qwen 雖有 9/11，但兩模型彼此只有 5/11；而且 qwen 對原評分器與規則 R 都同樣是 9/11。故本次交叉驗證**未能提供有力佐證**，不能把規則 R 的 S0 `13/42 → 16/42` 或 B1 相對落差 `−8 → −6` 當成已被獨立模型驗證的真實修正。

可保留的弱訊號只有：`18-Q6` 的兩個模型都支持 R 翻盤，與報告78 對「合計／累計」表面形式差異的診斷相容；但 `57-DIST2` 跨模型分歧，且其他原本未翻盤的 spans 也出現 granite 大量支持、qwen 大量不支持的差異。因此，§11 的正確定位仍是**評分器敏感度分析與診斷性下界／上界參考**：原始結果是 S0 K 13/42 對 B1 21/42（KG −8，`p=0.021484`），規則 R 結果是 S0 K 16/42 對 B1 22/42（KG −6，`p=0.109375`）；本次盲審沒有理由刪除任一組數字，也沒有理由宣稱 −6 比 −8 更接近真值。

本節因此不改寫 §9／§11 的主要結論，不回灌 `docs/論文/`，也不把任一模型判定升格為正式 scorer 或 production 預設。後續若要縮小不確定性，應對 `18-Q6`、`57-DIST2` 做人工法規語意 adjudication，並另行驗證 semantic span scorer；不能只依賴本次兩個小模型的盲審一致率。

---

## 13. GAP-S3-01 上下文組裝離線消融原型（2026-09-26）

### 13.1 T1–T2：最小消融方法與資料來源

本節依報告82 T1–T5 做**最小成本離線原型**，不是報告58 的完整架構實作。沒有建立 `ContextBundle`、沒有修改 `routers/agent.py::chat()`、沒有修改 `services/atomic_scorer.py`，也沒有連線或寫入 Neo4j；只透過既有生成 provider 呼叫本機 Ollama 的 `qwen2.5:7b`。

新腳本為 `scripts/eval/gap_s3_01_context_ablation.py`，使用 S0 K arm 的 `data/eval/baseline_runs/20260923_rebased/stage_s0_r1/records.json`，以及 B1 的 `data/eval/candidate_runs/s3_chunk_rag_b1_stage_a/records.json`。B1 原文段落沿用 §11.1 已示範的 `baseline_rag_index_236903cf-055a-40a8-8923-b9d06601f3b7_cs500.json`，依 `retrieved_chunk_ids` 離線還原；每題選取**包含目標 missing span 的最高順位 chunk**，只加入一個邏輯段落。選取結果如下：

| 題目 | B1 選取 chunk | B1 順位 |
| --- | --- | ---: |
| `18-Q1` | `N0030006_勞工請假規則_0` | 2 |
| `18-Q6` | `N0030006_勞工請假規則_1` | 1 |
| `57-AGGR14` | `N0090002_私立就業服務機構許可及管理辦法_2` | 1 |
| `57-COREF3` | `N0030006_勞工請假規則_1` | 1 |
| `57-DIST2` | `D0080015_警察人員特別休假辦法_0` | 1 |

K arm 原有的每個子問題 `prompt_context_lines` 完整保留，再附加：

```text
=== 依據法規原文段落 ===
【參考段落】（來源：<document/chunk>）
<一個 B1 還原段落>
```

複合題沿用 K arm 原本的子問題分解；同一個新增段落會附加到該題各個獨立子問題 prompt，這是維持既有 K 生成結構的一個邏輯段落，不是改成報告58 的 3–5 段提領策略。5 題各重複生成 2 次，共 10 次題目層級結果（複合題內共 22 個子問題 prompt）。每次完整答案與 prompt metadata 逐筆保存於 `data/eval/candidate_runs/gap_s3_01_context_ablation.json`。

### 13.2 T3：AtomicScorer 逐題比較

新答案使用既有 `AtomicScorer.evaluate()` 離線評分；本節的「目標 span 支持」嚴格指該 span 出現在 `AtomicScorer.supported_spans`，不把人工語意近似逕自升格為通過。原 S0 K 的 `is_perfect`／`missing_spans` 直接取自 frozen record，K+C 每次重生成則分別評分。

| 題目 | S0 K 原結果 | K+C run 1 | K+C run 2 |
| --- | --- | --- | --- |
| `18-Q1` | `is_perfect=False`；缺「住院者，二年內合計不得超過一年」 | `True`；目標 **支持**；`missing=[]` | `False`；目標 **支持**；仍缺「未住院者，一年內合計不得超過三十日」 |
| `18-Q6` | `is_perfect=False`；缺「一年內合計不得超過十四日」 | `True`；目標 **支持**；`missing=[]` | `True`；目標 **支持**；`missing=[]` |
| `57-AGGR14` | `is_perfect=False`；缺「從業人員人數逾十人者，應置就業服務專業人員至少三人，並自第十一人起，每逾十人應另增置就業服務專業人員一人。」 | `False`；目標 **不支持**；另缺五人以下、六至十人兩個級距 | `False`；目標 **不支持**；另缺五人以下、六至十人兩個級距 |
| `57-COREF3` | `is_perfect=False`；缺「勞工為親自照顧家庭成員，除本法或其他法律另有規定者外，得依前項規定請事假，並得擇定以小時為請假單位」 | `False`；目標 **支持**；仍缺「一年內合計不得超過十四日。事假期間不給工資」 | `False`；目標 **支持**；仍缺「一年內合計不得超過十四日。事假期間不給工資」 |
| `57-DIST2` | `is_perfect=False`；缺「給予一至三日之特別休假」 | `False`；目標 **不支持**；缺「當選直轄市、縣（市）級模範警察或好人好事代表者」及「給予一至三日之特別休假」 | `False`；目標 **不支持**；同上兩個 span |

彙總而言，10 次 K+C 生成中：

- 完整 `is_perfect=True`：**3/10**；其中只有 `18-Q6` 是 **2/2** 穩定完整通過，`18-Q1` 是 **1/2**。
- 原本缺失的目標 span 被 AtomicScorer 判為支持：**6/10**；按題目至少一次支持為 `18-Q1`、`18-Q6`、`57-COREF3`，共 **3/5** 題。
- `57-AGGR14` 與 `57-DIST2` 兩次均未取得 AtomicScorer 的目標 span 通過；因此不能把「把原文段落放進 prompt」描述成五題普遍有效。

需要特別註明 `57-AGGR14`：答案文字實際列出了三個級距及「自第十一人起，每逾十人應另增置…一人」，但模型把「從業」寫成簡體「从業」，所以逐字 `AtomicScorer` 仍將三個 exact span 都列為 missing；這是輸出表面形式問題，不應被誤報成該段落完全沒有被模型讀到。`57-DIST2` 也已用「可以獲得一至三日」表達相近法律效果，但未形成 gold 的逐字 span；而這個語意在 S0 原答案中本來就已大致出現，故不能算消融帶來的明確內容增益。

### 13.3 T4：副作用與生成穩定性檢查

| 題目 | S0 答案字元數 | K+C 兩次字元數 | 人工檢查摘要 |
| --- | ---: | ---: | --- |
| `18-Q1` | 558 | 995／983 | 原文段落讓住院上限被明確回答；但答案長度約增加 77%，兩次對未住院 30 日的表面形式不一致，且 run 1 加入較多工資／留職停薪延伸說明。 |
| `18-Q6` | 1035 | 547／547 | 長度反而下降約 47%，兩次結構一致且完整通過；本批最乾淨的正向訊號。 |
| `57-AGGR14` | 183 | 299／299 | 長度增加約 63%；答案列出完整級距，但隨後又說資料沒有分段對照，語意自我削弱，並受「从／從」表面差異影響評分。 |
| `57-COREF3` | 212 | 582／582 | 長度增加約 175%；目標長句被回答，但「有所不同」與後文「天數相同」的組織方式不夠乾淨，且 14 日與不給工資被拆開而未命中合併 gold span。 |
| `57-DIST2` | 306 | 290／336 | 長度平均約增加 2%；兩次都回答了全國性二至五日與直轄市／縣市級一至三日，但沒有改善 AtomicScorer 的目標 exact span，且內容與 S0 已有的語意回答沒有明顯新增。 |

本次沒有看到所有題目都因段落加入而爆量或完全失去結構，但 `18-Q1`、`57-AGGR14`、`57-COREF3` 已顯示：增加原文確實可能把遺漏事實帶進答案，卻同時增加答案冗長、表面形式變動或結論組織不穩定的風險。這是人工檢查到的副作用，不把「模型提到相關字句」等同於正式評分改善。

### 13.4 T5：是否投入報告58完整工程

本次結果對五題的初步判定是：**改善有限且不一致，目前證據不支持投入報告58的完整工程。** `18-Q6` 的 2/2 完整通過值得保留為局部訊號；`18-Q1` 只有 1/2 穩定，`57-COREF3` 只補到目標長句但仍非完整通過，`57-AGGR14`／`57-DIST2` 則沒有 AtomicScorer 層級的目標 span 改善。若把「語意上大致寫到」也算改善，仍會遇到 `57-AGGR14` 的表面評分失敗、`57-DIST2` 的基準答案已經具備相近內容，以及 COREF3 的結構副作用，不能據此合理化一次完整架構投入。

這不是宣稱報告58的雙軌上下文在所有題目、所有模型上都無效；樣本只有 5 題 × 2 次，且本原型只測「一個 B1 chunk」與既有 K prompt 的組合。但以本任務預先設定的投入門檻來看，**目前證據不支持投入** `ContextBundle`、Manifest source resolver 或 `chat()` 接線；除非後續有新的、較大樣本且控制好 scorer 表面形式與生成非決定性的不同比較證據，否則本方向暫緩，不設為任何預設機制，也不回灌 `docs/論文/`。
