...
13. ✅ **已完成（2026-09-19 已保存答案的自動配對評分與採用閘門）**：見 §4.18，boundary-swap NO-GO。
14. ✅ **已完成（2026-09-20 重啟「終止條件比較」子題）**：新增 `57-AGGR18`／`57-AGGR19`，題庫 65 題（verified 46）；發現原子評分不檢查歸屬。見 §4.19。
15. ✅ **已完成（2026-09-20~21 凍結基準評測）**：K arm、42 題、品質門檻式重複，**12/42 達標**（事後重算拒答 14/42）、跨文件題 0/13。見 §4.20、附錄D。
16. ✅ **已完成（2026-09-21 後續分析與查證）**：拒答事後重算、20 題檢索失敗歸因複核（35 個 span 只有 1 個真的沒抽到）見 §4.21；做法與文獻來源即時查證見報告61；RefusalBench 引用範圍已於程式碼與 11 處文件校正。
17. ⏭️ **下一階段**：見 [報告62 下一階段任務書](62_下一階段任務書_檢索排名與條文擴充驗證.md)。
