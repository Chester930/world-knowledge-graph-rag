...
| 題號 | atomic_accuracy | Context Recall | SNR | Chain Completeness | latency | 診斷 |
|---|---|---|---|---|---|---|
| `57-AGGR15` | 0%（0/2命中） | **100%** | **10.7%** | **100.0%（n=1）** | 125.3s | **Stage 3 生成端幻覺/平滑化**：兩個gold fact（子法§1授權句、母法§23第5項全文）皆已被正確檢索到（Context Recall 100%、Chain Completeness 100%），但LLM生成答案時未使用檢索到的正確法源條文，改以模糊敘述帶過——這是本任務C七組候選以來第一個「Context Quality完美但Atomic Accuracy為零」的案例，精確呈現報告57§2的「評測解耦」價值：若只看Atomic Accuracy會誤以為是檢索失敗，拆開看才能確診是純生成端問題 |
