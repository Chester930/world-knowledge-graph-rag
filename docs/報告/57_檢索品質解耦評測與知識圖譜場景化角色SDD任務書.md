...
- **新舊法終止條件比較子主題（§23/24/84/85）品質差**：§24全空（完全沒抽出任何Fact）、§23/§84皆漏抽「公立醫療機構認定」vs「中央衛生福利主管機關醫院評鑑合格醫院認定」這個題目原本設計要測的關鍵差異本身。**使用者裁示跳過不出這個子主題的題**——這是先核對再出題的紀律擋下一個實際上抽取地基不穩的題材，而非出了題才發現問題。⚠️2026-09-20補註：事後查佇列發現§23/§24（N0060041）與§84/§85/§86（N0050031）這幾個 chunk 在 `task_queue.db` 中皆為 `failed`（§24 現有 0 筆 Fact），「地基不穩」較可能是抽取執行失敗而非模型抽取品質，此結論的證據不足，見[附錄C](57_附錄C_KG重抽來源清單與失敗chunk盤點.md)。
