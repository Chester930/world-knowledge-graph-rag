...
- **✅ 補充：正式規模（`--runs 3`）跑測已完成（2026-09-13）**——同一 6 題（18-Q1~Q5、
  26-Q5）、同一 7 arm、`--runs 3`（126 筆呼叫），零錯誤。結果存
  `report39_comparison/formal_20260913/`（manifest.json／run1~3.json／summary.md）。
  抽樣核對 26-Q5 三個 run 的 K arm 答案：**三次都答對「六個月」、三次都答不出
  「當月一日」起算日**——確認 Q5 缺陷是系統性、非單次跑測的隨機雜訊。人工
  0/1/2 評分欄仍留空，尚待人工填寫才能產出報告 40 的最終結論。
- **✅ 補充：報告27 §6.2 T2 正式驗收測試已由另一位協作者（Gemini）獨立完成
  （2026-09-13，`run_t2_acceptance.py`，master `b15dd9d`）**——8 題 × 條件 A（KG）/D
  （純LLM）× 3 run＝48 筆，條件 A 在 5/8 題全數 3/3 通過、2 題部分通過、
  僅 26-Q5 對應題（Q5：跨文件不混淆）0/3（與本任務書的 26-Q5 缺陷發現一致）；
  條件 D 全部 0/3。與報告39 K arm 的發現互相印證。T2 不在報告39 SDD 範圍內，
  純供交叉參考。
- 已知 caveat：
  1. **前導版非正式版（已補齊正式規模）**：SDD-5 驗收本身跑的是「7 arm 跑通」
     （單一 6 題、`--runs 1`，見上）；2026-09-13 已補跑正式規模 `--runs 3`（見上），
     但**人工評分仍未填**——依 §4.1 的分批策略、多批交叉錨定、報告 40 產出，
     仍是使用者後續要做的事，非本任務書自動完成的範圍。
