...
>
> ⚠️ **誠實訂正（2026-08-18）**：第 4 節跨文件標準名提升機制（`_aggregate_alias_counts()`／`should_promote_by_frequency()`）程式碼本身確實已實作且有測試覆蓋，但**在正式環境從未真正被觸發執行過一次**——其資料來源 `HAS_ENTITY` 邊（`count(DISTINCT c.source_doc_id)`）依賴 `SVOTriple.source_doc_id`，而這個欄位在真正的抽取 Worker（`services/extraction_worker.py::_process_one()`）裡直到 2026-08-18 之前從未被賦值過，`merge_entity()` 因此一律走「跳過 `HAS_ENTITY` 邊建立」的分支。根因與修復見 `03_系統設計與方法論.md` § 3.1.4 §c——此修復僅對之後新產生的抽取資料生效，既有已完成的圖譜（例如勞基法語料）仍完全沒有 `HAS_ENTITY` 邊，第 4 節機制對這些既有資料依然不會生效，需重新抽取或另外回填。
