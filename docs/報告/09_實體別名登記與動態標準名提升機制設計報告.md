...
## 3. 線上動態勝負與標準名提升機制 (Dynamic Promotion)
...
由於文章最早出現的名稱往往是簡稱或縮寫（如首句出現 `I-35`，第 4 句才出現全稱 `Interstate Highway 35`），**文件內暫定標準名**不能死板固定於首次出現者，必須具備 **「動態優勝提升 (Frequency-Priority Promotion)」** 機制——但這只解決「這份文件自己內部怎麼稱呼才一致」，不解決「整個知識圖譜該用哪個名稱」。
...
flowchart TD
    ...
    INCR --> COMPARE{"此別名目前的出現次數<br/>是否 > 現有標準名 Key 的出現次數？"}
    
    COMPARE -- "是（更常出現）" --> PROMOTE["觸發標準名提升 (Promotion)：<br/>1. 繼承舊條目之 occurrences 與 alias_counts<br/>2. 舊 Key 降級收錄入 alias_counts<br/>3. 主 Key 升級更新為此別名"]
    COMPARE -- "否（次數較低）" --> TIELEN{"次數相同（平手）時：<br/>此別名長度 是否 > 現有 Key 長度？"}
    TIELEN -- "是" --> PROMOTE
    TIELEN -- "否或仍平手" --> KEEP["保持既有 Key 不變<br/>（次數已於 INCR 步驟更新）"]
...
### 3.2 勝負比試規則（2026-07-21 修訂：頻率優先 + 長度次要）

1. **出現頻率優先 (Frequency Priority，主規則)**：
   - 範例：文件內「泰國」出現 50 次、罕見的正式全名只出現 2 次 $\rightarrow$ **頻率高者勝**，避免長但罕用的形式被誤選為標準名。
   - 文獻依據：Wikidata「社群共識最常用名稱」（Vrandečić & Krötzsch, 2014）、CESI「頻率加權質心」（Vashishth et al., 2018）——兩者皆為頻率導向，見第 6 節。
2. **平手時長度優先 (Length as Tie-breaker，次規則，僅頻率相同時觸發)**：
   - 範例：`"Interstate Highway 35"` 與 `"I-35"` 若出現次數恰好相同 $\rightarrow$ 取較長/較完整者。
   - **此規則本身查無直接文獻先例**，僅呼應學界零散提及的「取最長 mention」啟發式（詳見第 6 節誠實聲明），性質上屬本論文自行設計的簡化 tie-breaker，非文獻既有方法。
3. **仍平手則保留既有 Key (最終 Tie-Breaker)**：
   - 範例：`"Apple Inc."` ⚔️ `"Apple Corp."` 若頻率與長度皆相同 $\rightarrow$ **維持既有 Key 不變**，新名稱納入 `alias_counts`。
...
ORDER BY freq DESC
```
若聚合結果顯示某個別名的**跨文件累積次數**已超過目前 `Entity.name` 的累積次數，就把 `Entity.name` 更新為該別名（規則與第 3 節相同：頻率優先，次數相同才比長度，再相同保留現狀），原名稱降級收錄進 `aliases`。**這才是本機制的真正權威判斷**——同一實體在夠多不同文件中都被稱為某個名稱，才會讓它成為標準名；單一文件的局部偏好不會直接污染整個圖譜。
...
- [ ] **`services/entity_registry_service.py`**（**文件內範圍**，第 3 節）
  - `EntityRegistry`: 負責維護文件內 `registry` 狀態與 $O(1)$ 字典 key 升級。
  - `should_promote(mention_count, current_key_count, mention, current_key) -> bool`（2026-07-21 修訂，取代原 `is_better_canonical_name(name_a, name_b)`）: 實作頻率優先＋長度次要的比對邏輯，簽章新增出現次數參數。**此函式只產生「文件內暫定標準名」，不直接寫入 Entity.name。**
  - `resolve_alias(mention, registry) -> str`: 回傳對應的文件內暫定標準名。
- [ ] **`services/svo_service.py` 新增跨文件標準名重算**（**跨文件範圍，第 4.3 節，真正權威層**）
  - `recompute_canonical_name(entity_id) -> str`（2026-07-21 新增）：查詢該實體所有 `HAS_ENTITY` 邊的 `surface_form` 聚合次數，套用與 `should_promote()` 相同的頻率優先規則，決定是否更新 `Entity.name`。
- [ ] **與 `services/svo_service.py` 寫入銜接**
  - 將 `registry` 的 `alias_counts` 字典傳遞至 Neo4j `MERGE` 語句中進行動態合併（持久化時仍可攤平為 `aliases` 陣列，見第 4 節）。
...
### 6.2 規則層佐證：出現頻率優先的標準名選取

第 3 節「出現頻率優先」規則的文獻依據——這兩篇已收錄於 `docs/參考文獻/03_資訊抽取與本體設計/`，本報告直接沿用，非新增下載：

1. **Vrandečić & Krötzsch (2014)**，*Wikidata: A Free Collaborative Knowledgebase*，CACM 57(10)——label（權威標準名）的選取依據明確是「社群共識中最常用的名稱」，與本報告「頻率優先」規則方向一致。
...
**誠實侷限**：兩篇皆非「頻率優先、長度僅作平手次要規則」這個完整組合的直接出處，只確認「頻率導向選代表名稱」這個大方向站得住腳；「長度作為平手 tie-breaker」這一條細則本身仍查無直接文獻先例，屬本論文自行設計的簡化規則（詳見第 3.2 節）。
