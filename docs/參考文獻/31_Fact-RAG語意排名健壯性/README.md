...
- [x] BM25 對本專案 `Fact` 節點這種短文字（10-30 字 SVO 串接）的實際鑑別力，未有現成文獻直接量化——**2026-09-14 已用 WebSearch 補查證**（查詢「短文本/命題級 BM25 鑑別力」「三元組層級 hybrid 檢索」），沒有找到比 Louis et al./Reuter et al. 更貼近的既有結論，只查到 BM25＋dense RRF 融合在法律領域的一般性確認（無新增可引用文獻）。`docs/參考文獻/08_向量化與語意表示/chen-et-al-2023-dense-x-retrieval.pdf`（Dense X Retrieval，已收錄）確立「proposition／細粒度檢索單元」是研究過的合理粒度，但只涵蓋 dense 檢索，不涵蓋 sparse。**結論維持不變：這是本專案需要自行實測校準的部分，非文獻既有結論**，已記入 `docs/論文/02_文獻探討.md` §2.6.5.1「誠實的文獻缺口」段落。
