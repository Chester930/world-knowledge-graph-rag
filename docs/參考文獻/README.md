...
| `19_生成端長清單事實遺漏與位置偏誤/` | `docs/報告/22_乾淨重跑新舊KG問答品質比對報告.md` § 5、§5.1；`routers/agent.py::_sort_lines_by_relevance()`（低成本第一版已實作，commit `e4e49a8`；真實線上驗證效果有限） | ✅ 低成本緩解已實作；🟢 已下載 1 篇（TACL 2024／arXiv 2307.03172，Lost in the Middle），文獻現象與真實觀察吻合；另查得LangChain `LongContextReorder` 開源實作可直接參考演算法邏輯 |
