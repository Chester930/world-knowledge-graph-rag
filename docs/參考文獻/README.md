...
| `04_圖遍歷與大節點問題/` | 02_文獻探討.md § 2.4.8（RQ6）；`docs/報告/27_圖遍歷向量引導剪枝機制設計報告.md`（報告26 §4 #4 觸發，RQ6 轉分層實作設計） | 📋 本資料夾不另存 PDF，僅 README 記角色＋交叉引用：CatRAG（`02_`）、G-Retriever（`12_`）、PathRAG／LightRAG（`02_`）、Han 綜述／SAGE（`21_`）；CatRAG 方法章節待精讀 |
...
| `18_抽取數值忠實性核對/` | `docs/報告/20_抽取數值忠實性核對機制設計報告.md`＋`docs/報告/25` §4 發現3（皆✅已實作）；03_系統設計與方法論.md § 3.1.3／§ 3.1.4 | ✅ 字詞層級（commit `e45eda6`）＋子句層級綁定（2026-09-03，報告25 發現3）皆已實作（`services/svo_service.py`）；🟢 精讀 1 篇（ACL 2017）＋🟡 精讀 2 篇（SRVF arXiv:2412.07289 / atomic-propositions arXiv:2604.02866）＋🟡 已下載 1 篇（GenRES arXiv:2402.10744）；Huang et al. 2026 entailment-aware 無免費全文、僅書目；部落格 1 篇（Thinking Machines Lab，僅書目） |
...
| `morris-et-al-2023-levels-of-agi.pdf` | Morris et al. (2024；arXiv 2023, Google DeepMind), ICML 2024 | arXiv:2311.02462 |
| `morris-et-al-2023-levels-of-agi-v5.pdf` | Morris et al. (2024；v5 2025 最新修訂版) | arXiv:2311.02462v5 |
...
| `legg-et-al-2026-from-agi-to-asi.pdf` | Genewein et al. (2026, Google DeepMind), *From AGI to ASI* | arXiv:2606.12683 |
...
| `mihindukulasooriya-et-al-2020-sling-relation-linking.pdf` | Mihindukulasooriya et al. (2020), *SLING*，ISWC 2020（關係連結任務佐證，供 3.2 §b 使用） | [arXiv:2009.07726](https://arxiv.org/abs/2009.07726)；[GitHub IBM/kbqa-relation-linking](https://github.com/IBM/kbqa-relation-linking)（22★，2026-07-26 訂正原誤植的 `IBM/neuro-symbolic-ai`；⚠️ 2026-08-19 複查已 archived） |
...
> 起點：報告18追加驗證發現規則7（few-shot 範例式修補）遇到範例沒示範過的附加規定類別（如「以小時為請假單位」）仍會漏抓，使用者指出這違背通用架構的初衷。報告19提議借用報告16事實接地性核對機制的架構精神，鏡像到抽取完整性方向；本次查證的兩篇文獻是該設計 v2（涵蓋比對＋針對性補抽）的直接方法論來源，詳細角色見各自條目。**設計尚未實作**，見 `docs/報告/19_SVO抽取完整性自我核對機制設計報告.md` §7 待確認事項。

| 檔案 | 文獻 | 來源 | 角色 |
|---|---|---|---|
| `yang-et-al-2026-promem.pdf` | Yang, Sun, Wei & Hu (2026), *Beyond Static Summarization: Proactive Memory Extraction for LLM Agents* | arXiv:[2601.04463](https://arxiv.org/abs/2601.04463) | ✅ 已下載並精讀全文（2026-08-31）；🟡 尚未查證同行評審，僅2026年1月arXiv預印本。其 Memory Completion 演算法（對每個來源單位算與已抽記憶項目的cosine相似度，`>τmatch=0.6`視為已涵蓋，未涵蓋單位才觸發第二次LLM抽取）是報告19 §3設計的直接方法論來源，抽取對象由對話輪改為法規原文句子。消融實驗證實此步驟單獨貢獻 Memory Integrity 54.03%→73.80%。⚠️ 門檻值0.6未針對本專案中文法規語料校正；未見公開程式碼倉庫 |
| `liu-et-al-2025-verifact.pdf` | Liu, Zhang, Munir, Gu & Wang (2025), *VeriFact: Enhancing Long-Form Factuality Evaluation with Refined Fact Extraction and Reference Facts*，v2 (2025-09-26) | arXiv:[2505.09701](https://arxiv.org/abs/2505.09701) | ✅ 已下載並精讀全文（2026-08-31）；🟡 尚未查證同行評審，仍為arXiv預印本。用word-mapping（最長公共子串，非LLM）定位原文中未被抽取結果涵蓋的片段，再用LLM判斷是否構成有意義的遺漏並補寫；量化效果：不完整事實比例56.7%→22.5%、遺漏事實數1.22→0.76、人類事實覆蓋率77.5%→87.1%。⚠️ word-mapping為字串比對演算法，與報告19採用的embedding相似度比對機制不同，僅佐證「兩階段：定位→補完」大方向，具體演算法不可直接套用 |

**對照組（不列入正式文獻，僅供設計取捨參考）**：Google `google/langextract`（開源專案，即時查證 **38,512★**，`gh api`，2026-08-31，`archived: false`）——其 `extraction_passes=N` 參數用N次獨立完整重抽再合併（ensemble式）解決長文件召回問題，無定位遺漏片段步驟，是報告19刻意不採用的暴力對照做法，見報告19 §3「比v1好在哪」。
