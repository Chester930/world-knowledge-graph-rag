...
| `13_實體節點向量化去重開源專案/` | 03_系統設計與方法論.md § 3.1.4「實體對齊/去重」（`DEDUP4`／`resolve_entity_name()`，Entity 節點 `name_embedding` 效能改造，2026-08-03 已定案即將實作） | ✅ 機制已定案；文獻查證仍為 🟡 初步層級，已下載 2 篇，另查證 3 個 1000★+ 開源專案，皆僅初步查證未全文精讀 |
