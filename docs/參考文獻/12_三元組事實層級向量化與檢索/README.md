對應 `../../論文/03_系統設計與方法論.md` § 3.1.4 §a「事實層級向量化 Behavior Tree（三元組向量化）」——查詢時對三元組本身做語意層級檢索的設計。**狀態（2026-08-03 更新）**：✅ 即時路徑（`Fact` 節點建立、向量索引、`vector_search_facts()` 檢索函式）已實作，`services/svo_service.py`；🟡 §b 歷史 citation 回填批次任務、檢索結果接入回答階段 Context 組裝流程，仍為設計提案，尚未實作。
