...
> **狀態：🟡 初步蒐集，機制本身尚未定案**。本次查證僅確認來源真實性（`gh api` 驗證 star 數、下載可公開取得的全文 PDF），**尚未逐字精讀全文**，內容摘要僅供篩選判斷，不可直接當作精讀後的正式引用依據——比照 `12_三元組事實層級向量化與檢索/` 從「初次查證」到「全文精讀」的兩階段模式，本資料夾目前處於第一階段。
