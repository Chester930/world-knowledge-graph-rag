...
> **狀態：✅ 2026-08-03 使用者確認定案（新增 `name_embedding` 屬性＋fallback 漸進部署），即將實作；🟡 向量索引 KNN 查詢部分刻意排除於本次範圍，留待後續**。完整設計定案內容見 `03_系統設計與方法論.md` § 3.1.4 `DEDUP4` 段落新增的「Entity 節點向量化效能改造」註記。本資料夾內容仍為初次查證層級（`gh api` 驗證 star 數、下載可公開取得的全文 PDF，**尚未逐字精讀全文**）——機制本身已定案不代表文獻已達精讀標準，兩者是獨立的判準，比照 `12_三元組事實層級向量化與檢索/` 從「初次查證」到「全文精讀」的兩階段模式，本資料夾文獻查證仍處於第一階段，若後續要提升信任分級需另外補齊全文精讀。
