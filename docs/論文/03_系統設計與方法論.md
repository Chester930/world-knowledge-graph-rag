...
flowchart TD
    ...
    STDSENTS --> SENTEMBED["逐句計算 embedding<br/>（標準化句子，非 3.1.1 的 RAG chunk 向量）"]
...
>
> ✅ **`CHUNKREADY` 端到端串接（2026-07-21 新增）**：`services/svo_preprocessing_service.py::prepare_svo_ready_chunks()` 把 `get_or_rebuild_sentences()`（`GETSENT`）→ `entity_registry_service.apply_registry()`（`REGISTRY`／`PROMOTE`，可選）→ `pronoun_resolution_service.resolve_coreference_pipeline()`（`PRONCHECK`／`PRONLLM`）→ `svo_chunking.build_svo_chunks()`＋`write_svo_chunks()`（`SVOGROUP`）串成一條真正可呼叫的管線，測試見 `tests/services/test_svo_preprocessing_service.py`（3 項）。**⚠️ 誠實侷限（尚未解決）**：具名提及抽取（NER）不在任何既有模組職責範圍內，是尚待補齊的上游依賴——`prepare_svo_ready_chunks()` 目前唯一可行的呼叫方式是 `mentions=None`，此時跳過整個 §a 別名登記表階段，只完成代名詞消解，未完成別名收斂，這是系統目前的真實狀態，非刻意簡化。
>
...
>
> ✅ **`SENTEMBED` 逐句 embedding（2026-07-21 定案，實作仍待補）**：標準化句子清單產出後，逐句計算 embedding（與 3.1.1 的 RAG chunk 向量分開計算、分開索引，粒度不同）。這一步本身已定案，但用途分兩個層次：① 作為後續 SVO 切塊分組的候選依據來源之一（例如未來若採語意斷點式分組，這裡的向量即可直接使用），但**分組規則本身已定案採簡單固定聚合，非語意切分**（見下方 `SVOGROUP` 註記）；② 讓 `docs/報告/08_三軌混合檢索架構與標準化RAG設計報告.md` 提出的「標準化 RAG」檢索軌道具備可實測的基礎建設，第五章消融實驗可據此**實際比較**傳統 RAG 與標準化 RAG 的效果，而非僅依賴文獻宣稱——08 報告本身的三軌架構是否納入 RQ1/RQ2 既有範圍仍需另外討論（見 `docs/報告/06_SVO抽取管線調整任務書.md` 第 5 節）。**實作現況**：`SENTEMBED` 逐句 embedding 呼叫本身尚未實作；但下游的 `PROMOTE`（`entity_registry_service.py`）與 `SVOGROUP`（`svo_chunking.py`）已實作完成，不依賴 `SENTEMBED` 的向量，兩者互不阻塞。
>
