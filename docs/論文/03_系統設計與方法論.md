...
> ✅ **實作（2026-07-21）**：`routers/staging.py::_trigger_extraction()` 在 `assign()`／`confirm_cluster()`／`classify()`（自動分配部分）三個實際觸發文件搬移的端點之後，同步直接呼叫 `CHUNKREADY`（`prepare_svo_ready_chunks()`）＋`ENQUEUE`（`task_queue_service.enqueue()`），符合本段「立即觸發」的敘述，同步呼叫（而非背景排程）是 2026-07-21 使用者決策。測試見 `tests/routers/test_staging.py`。
