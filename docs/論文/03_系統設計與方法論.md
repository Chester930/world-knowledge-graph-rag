...
### 3.1.2 抽取任務佇列與斷點續傳（抽取視窗粒度）

> 🟡 **本節粒度定義正在修訂中（2026-07-20 討論起）**：原設計以「chunk」（3.1.1 已存在的 500 字元切塊）同時作為向量索引與 SVO 抽取的追蹤單位；重新檢視 3.1.1 輸出到 SVO 抽取開始之間的銜接後，確認這兩個用途的最佳粒度不同——向量檢索需要細粒度（句子級，避免雜訊稀釋相似度），SVO 抽取需要粗粒度（多句組成的滑動視窗，LLM 才有足夠上下文做指代消解）。粒度整合的完整設計見下方 3.1.2 §a（草稿）；本節主圖已將追蹤單位由「chunk_index」改標註為「抽取視窗」，但視窗編號方式、`task_queue.db` schema、跨文件排序規則等細節仍待收斂，見 §a 末尾待決策清單。
...
flowchart TD
    ...
    INITREC --> WINDOWPREP
    MIGRATEVEC --> WINDOWPREP

    WINDOWPREP[["切塊粒度整合：句子級向量索引＋SVO 抽取滑動視窗<br/>（chunk 本身在 3.1.1 之前已切好，這裡不重切）<br/>— 見 3.1.2 §a Behavior Tree（🟡 草稿，2026-07-20 討論中）"]] --> ENQUEUE["登記進 task_queue.db<br/>⚠️ 追蹤單位從 chunk_index 改為 §a 定義的抽取視窗，<br/>schema 與「只登記尚未完成」的判斷方式待 §a 收斂後更新"]
    ENQUEUE --> WORKER["背景 Worker 依序<br/>取出下一個 pending 抽取視窗<br/>⚠️ 同文件內順序、跨文件排序規則見 §a 待決策清單"]
    ...
    TRUST -->|是| SCAN["查 task_queue.db<br/>找出尚未 completed 的最早抽取視窗<br/>⚠️ 視窗排序依據見 §a 待決策清單"]
...
### 3.1.2 §a：切塊粒度整合——向量索引 vs. SVO 抽取視窗（🟡 草稿，2026-07-20）
...
> 7. **（新增）滑動視窗 vs. 指代消解前置——選一個，還是兩者並用？** CORE-KG 的指代消解前置法（見上方文獻查證）與本節原本的滑動視窗法，都是回應「跨句指代消解導致漏抽」這同一個問題的不同技術路線，兩者互斥或互補尚未評估；若採用指代消解前置，§a 現有的滑動視窗設計可能整段不需要（切塊可以直接沿用原有 chunk，指代消解在切塊前先做一次即可），這會大幅簡化本節設計，需要認真評估後才能定案。
