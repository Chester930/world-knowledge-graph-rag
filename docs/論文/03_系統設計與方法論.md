...
>
> ✅ **`SENTEMBED` 逐句 embedding（2026-07-21 定案）**：標準化句子清單產出後，逐句計算 embedding（與 3.1.1 的 RAG chunk 向量分開計算、分開索引，粒度不同）。這一步本身已定案，但用途分兩個層次：① 作為後續 SVO 切塊分組的候選依據來源之一（例如未來若採語意斷點式分組，這裡的向量即可直接使用），但**分組規則本身仍未定案**，不代表 SVO 切塊已改採語意切分；② 讓 `docs/報告/08_三軌混合檢索架構與標準化RAG設計報告.md` 提出的「標準化 RAG」檢索軌道具備可實測的基礎建設，第五章消融實驗可據此**實際比較**傳統 RAG 與標準化 RAG 的效果，而非僅依賴文獻宣稱——08 報告本身的三軌架構是否納入 RQ1/RQ2 既有範圍仍需另外討論（見 `docs/報告/06_SVO抽取管線調整任務書.md` 第 5 節）。**實作現況**：3.4 §a 本身尚未實作（`svo_service.py` 現況為 stub），`SENTEMBED` 目前僅為設計定案，程式碼待 3.4 §a 動工時一併補上。
>
> 🟡 **`SVOGROUP` 組塊上限（2026-07-21 暫定，非最終定案）**：先採簡單固定聚合（依序聚合句子，最多 5 句或最多 300 字元，先到者為準），語意斷點式分組（07 報告的核心演算法）因文獻支撐不足暫緩（見 `03_變更紀錄.md` 與 `06_SVO抽取管線調整任務書.md` 第 5 節的查證過程）。300 字元這個數字沒有直接文獻依據——GraphRAG（Edge et al., 2024）附錄查證到的方向性發現是「切塊越小、抽取密度越高」，其測試範圍是 600/1200/2400 **token**（非中文字元，換算不精確，僅供粗略參考），比 3.1.1 現行 500 字元的 RAG 切塊更小的方向站得住腳，但具體數值本身是工程預設，非嚴謹推導。**此參數暫定，後續 3.4 節其餘討論（例如是否需要重疊、與 §b 跨文件別名消解的銜接）可能導致調整**，最終數值留給第五章消融實驗校準。
...
>
> ⚠️ **待決策**：`ESCALATE` 的呼叫時機（embedding 相似度落在哪個區間才算「灰色地帶」），仍尚未定案，需留給第四章實作與第五章消融實驗。`Entity.name` 選取規則已如上定案，但**尚未決定的是儲存方式**——究竟持久化一份 `Entity.aliases` 陣列屬性（09 報告原提案），還是維持純粹由 `surface_form` 邊屬性動態查詢衍生（不新增節點屬性），兩者取捨不同，留待後續討論定案。本節與 §a 皆為設計提案，非定案實作（`svo_service.py`／DEDUP 相關程式碼現況為 stub）。
