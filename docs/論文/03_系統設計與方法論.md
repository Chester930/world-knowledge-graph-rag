...
flowchart TD
    ...
    ALIASRULE --> PROMOTE
    ALIASLLM --> PROMOTE
    PROMOTE{"動態標準名提升：<br/>此別名出現次數 +1 後<br/>是否 ＞ 現有標準名 Key 的出現次數？<br/>（次數相同則比字面長度，再相同則保留既有 Key）<br/>— 見 docs/報告/09_實體別名登記與動態標準名提升機制設計報告.md"}
...
>
> **`PROMOTE` 動態標準名提升機制（2026-07-21 新增，完整設計見 `docs/報告/09_實體別名登記與動態標準名提升機制設計報告.md`）**：`ALIASCHECK` 判定某個提及對應既有登記表條目後，並非直接沿用該條目現有的標準名——文件最早出現的字面形式常是簡稱或縮寫（如首句只出現「I-35」，第 4 句才出現全稱「Interstate Highway 35」），標準名不能死板固定於首次出現者。本機制採**出現頻率優先**：登記表額外記錄每個別名各自的出現次數，當前提及所屬別名的次數更新後若超過現有標準名 Key 的次數，就把 Key 換成這個更常出現的字面形式，原 Key 降級收錄進登記表；次數相同才比字面長度（次規則），再相同則保留既有 Key（最終 tie-breaker）。**查證結果與誠實框架**：此機制原提案（09 報告初稿）以「長度優先」為主規則並宣稱 7 項專案/文獻佐證，經 2026-07-21 查證後發現全部不支持該具體演算法組合，其中 3 項是明確採用不同方法（詳見 `docs/參考文獻/10_跨文件實體別名消解與增量聚類/README.md`）。現版「頻率優先」規則改為呼應 Wikidata（Vrandečić & Krötzsch, 2014）「社群共識最常用名稱」與 CESI（Vashishth et al., 2018）「頻率加權質心」兩篇既有查證文獻的方向（皆收錄於 `docs/參考文獻/03_資訊抽取與本體設計/`）——但**這兩篇文獻只佐證「頻率導向選代表名稱」這個大方向，不是本機制「頻率優先＋長度平手次規則」這個完整組合的直接出處**；長度 tie-breaker 本身查無文獻先例，依 3.7 節誠實框架，應標註為**本論文自行設計的工程決策**，而非文獻既有方法的複製。
>
> ⚠️ **本節輸出僅為「文件內暫定標準名」，非最終權威（2026-07-21 再修訂，重要）**：`PROMOTE` 的頻率統計範圍**僅限單一文件**——單一文件內某個別名出現次數多，可能只反映這份文件/這位作者的行文習慣（範圍內簡稱），不代表這是實體在整個語料庫中被廣泛認可的通用標準名稱。若把本節輸出直接當成最終寫入知識圖譜的 `Entity.name`，標準名會隨文件來源飄移，違背 RQ4b「跨文件語意一致」的核心目標。因此本節輸出**只用於這份文件自身的處理**（下游代名詞消解、SVO 切塊分組），最終寫入圖譜的 `Entity.name` 由 §b「跨文件別名頻率聚合」決定，且隨新文件持續加入可能修正——兩者衡量範圍不同、權威層級不同，不可混淆。
>
> **與 05 任務書的分工（2026-07-21 補上實作，改用 10 報告的雙軌偵測）**：代名詞消解（`PRONCHECK`／`PRONLLM` 節點）完整規格見 `docs/報告/05_指代消解與前處理任務書.md`（雙向上下文視窗設計、Prompt 模板）與 `docs/報告/10_代名詞雙軌檢測與正則詞庫自動進化機制設計報告.md`（POS＋正則雙軌偵測，2026-07-21 使用者決策採用，取代 05 單一正則設計已知的「其/該」誤觸發限制）。**✅ 已實作**：`services/pronoun_resolution_service.py`——`detect_pronoun()`（雙軌三路分流：正則命中／POS 命中但正則未收錄／皆未命中）、`SpacyPosTagger`（spaCy `PRON`/`DET` 標籤，透過 `PosTagger` 介面注入，未安裝 spaCy 時不影響模組其餘邏輯匯入與測試）、`audit_unmapped_pronoun()`（背景 LLM 詞庫審核）、`resolve_coreference_pipeline()`（前 4 後 2 雙向上下文消解主體，`llm_provider=None` 時原句直接通過）。測試見 `tests/services/test_pronoun_resolution_service.py`（18 項，皆以 Fake `PosTagger` 驗證雙軌判讀邏輯，真實 spaCy 整合本身未在本專案環境驗證，見 `requirements.txt` 新增項目）。**別名登記/比對機制（`REGISTRY`／`ALIASCHECK`／`ALIASLLM`／`PROMOTE`）已實作**：`services/entity_registry_service.py`（`EntityRegistry` 類別、`should_promote()` 頻率優先規則、`_is_rule_match()` 子字串/縮寫規則、`apply_registry()` 整合入口、`write_registry_snapshot()`／`read_registry_snapshot()` 斷點續傳快照），測試見 `tests/services/test_entity_registry_service.py`（17 項）。
>
...
>
> **§a 的輸出不是最終標準名——§b 才是真正的權威層（2026-07-21 修訂，重要）**：§a `PROMOTE` 產生的是「文件內暫定標準名」，衡量範圍僅限單一文件，只服務該文件自身的一致性（代名詞消解、SVO 切塊分組）；單一文件內某個別名出現次數多，可能只反映這份文件/這位作者的行文習慣（範圍內簡稱），不代表這是實體在整個語料庫中被廣泛認可的通用標準名。若直接把 §a 的暫定名稱當成最終 `Entity.name`，標準名會隨文件來源飄移，破壞跨文件語意一致性——這正是 RQ4b 要解決的問題本身，不能在自己的機制裡留這個漏洞。因此 `Entity.name` 的真正權威判斷交給本節：**每次合併三元組後（`RECHECK` 節點），重新聚合該實體所有 `HAS_ENTITY` 邊的 `surface_form` 跨文件累積次數**，與 §a 相同的規則（頻率優先，次數相同才比長度，再相同保留現狀）在此重新套用一次，只是資料來源從單一文件的登記表換成跨文件累積統計；若某別名的跨文件累積次數已超過現有 `Entity.name`，才觸發 `UPDATENAME` 更新，原名稱降級收錄進別名。同一實體要在夠多不同文件中都被稱為某個名稱，才會讓它成為標準名。
>
> **記錄別名而非只是靜默合併**：`(Chunk)-[:HAS_ENTITY]->(Entity)` 邊（`docs/ARCHITECTURE.md`，HippoRAG 文獻背書）新增 `surface_form` 屬性，記錄每次提及實際使用的字面——這份逐邊記錄正是 `RECHECK` 重新計算跨文件標準名的資料來源，查詢 `MATCH (c)-[r:HAS_ENTITY]->(e) RETURN DISTINCT r.surface_form, count(*)` 即可重算，不需新增節點類型，直接沿用既有 schema。這同時滿足「別名要合併」「別名要可追溯」「標準名選取跨文件一致」三個要求，服務 RQ3／RQ4a／RQ4b 共同的可追溯性（AIS）次要指標。
>
...
>
> ✅ **實作現況（2026-07-21 定案；同日再修訂：改以 HAS_ENTITY 邊聚合，修正與文字描述不一致之處）**：`DEDUP3`／`ESCALATE`／`RECORD3B`／`RECHECK`／`UPDATENAME` 已實作於 `services/svo_service.py`——`resolve_entity_name()`（編輯距離→cosine→LLM 仲裁三段式判斷）、`_merge_chunk_mention()`（建立 `Chunk` 節點與 `HAS_ENTITY {surface_form}` 邊，即上方「記錄別名而非只是靜默合併」段落描述的實際落地）、`_aggregate_alias_counts()`（對應上方 `MATCH (c)-[r:HAS_ENTITY]->(e) RETURN r.surface_form, count(*)` 查詢範例）、`merge_entity()`（串連三者＋跨文件標準名動態更新）。門檻常數定義於 `core/constants.py`（`ENTITY_DEDUP_EDIT_RATIO_THRESHOLD`／`ENTITY_DEDUP_COSINE_THRESHOLD`／`ENTITY_DEDUP_ESCALATE_LOW_THRESHOLD`，皆為未驗證工程預設）。測試見 `tests/services/test_svo_service.py`（`InMemoryEntityDriver` 模擬 Entity／Chunk 節點與 HAS_ENTITY 邊的完整 MERGE／改名／聚合狀態，共 16 項）。`Entity.aliases` 已實作為節點陣列屬性（2026-07-21 使用者決策），內容為聚合查詢當下的別名去重清單（快取用途，權威來源仍是 `HAS_ENTITY` 邊本身，而非這個陣列屬性）；缺乏 chunk 追溯資訊（`source_doc_id`／`source_svo_chunk_index`）的呼叫會跳過 `HAS_ENTITY` 邊建立與標準名提升判斷，只單純 MERGE 節點，此為刻意的保守退化，非 bug。
>
