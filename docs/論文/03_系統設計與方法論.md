...
>
> **與 05 任務書的分工**：代名詞消解（`PRONLLM` 節點）完整規格見 `docs/報告/05_指代消解與前處理任務書.md`（正則清單、雙向上下文視窗設計、Prompt 模板、Python 實作），本節只展示其在整條前處理管線中的位置。**別名登記/比對機制（`REGISTRY`／`ALIASCHECK`／`ALIASLLM`／`PROMOTE`）已實作**：`services/entity_registry_service.py`（`EntityRegistry` 類別、`should_promote()` 頻率優先規則、`_is_rule_match()` 子字串/縮寫規則、`apply_registry()` 整合入口、`write_registry_snapshot()`／`read_registry_snapshot()` 斷點續傳快照），測試見 `tests/services/test_entity_registry_service.py`（17 項）。**尚未整合的部分**：具名提及本身的抽取（NER，本模組的輸入前提）與代名詞消解模組（05 任務書）的端到端串接，仍是後續整合工作，非本模組職責範圍。
>
