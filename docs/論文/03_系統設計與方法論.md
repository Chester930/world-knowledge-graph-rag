...
> 4. **HDBSCAN 未做降維前處理**：見上方 BERTopic 可信任專案背書段落——`cluster_vectors()` 直接對原始高維 embedding 做歐氏距離分群，未如業界標準管線先做 UMAP 降維。處理方式見 §a 末尾「降維前處理的決策」。
...
> 6. **未分配資料夾池的擴展性上限未討論**：`mean_pairwise_cosine_similarity()` 與 HDBSCAN 的距離計算皆為 O(n²)，而 3.1.1 §a 原文已承認未分配資料夾池「隨時間持續累積」，卻未討論大規模時的效能上限。處理方式見 §a 末尾「擴展性上限的決策」。
>
