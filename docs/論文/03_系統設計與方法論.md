...
>
> ✅ **實作現況（2026-07-21）**：`DEDUP3`／`ESCALATE`／`RECHECK`／`UPDATENAME` 已實作於 `services/svo_service.py`——`resolve_entity_name()`（編輯距離→cosine→LLM 仲裁三段式判斷）、`merge_entity()`（含跨文件標準名動態更新），門檻常數定義於 `core/constants.py`（`ENTITY_DEDUP_EDIT_RATIO_THRESHOLD`／`ENTITY_DEDUP_COSINE_THRESHOLD`／`ENTITY_DEDUP_ESCALATE_LOW_THRESHOLD`）。測試見 `tests/services/test_svo_service.py`（含 `InMemoryEntityDriver` 模擬完整 MERGE／改名狀態，共 14 項）。`Entity.aliases`／`alias_counts_json` 已實作為節點陣列/JSON 字串屬性（原「陣列屬性 vs. 由 surface_form 邊動態衍生」待決策，2026-07-21 使用者決策採陣列屬性）。
>
