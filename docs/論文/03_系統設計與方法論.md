...
>
> ✅ **`CHUNKREADY` 端到端串接（2026-07-21 新增；2026-07-22 補上 `SENTEMBED`）**：`services/svo_preprocessing_service.py::prepare_svo_ready_chunks()` 把 `get_or_rebuild_sentences()`（`GETSENT`）→ `entity_registry_service.apply_registry()`（`REGISTRY`／`PROMOTE`，可選）→ `pronoun_resolution_service.resolve_coreference_pipeline()`（`PRONCHECK`／`PRONLLM`）→ `write_sentence_embeddings()`（`SENTEMBED`，`embedding_provider` 提供時才執行）→ `svo_chunking.build_svo_chunks()`＋`write_svo_chunks()`（`SVOGROUP`）串成一條真正可呼叫的管線，測試見 `tests/services/test_svo_preprocessing_service.py`（5 項）。`routers/staging.py::_trigger_extraction()` 已接上同一個 `embedding_provider`（與 `EMBEDCHUNK` 共用同一次 `get_embedding_provider()` 呼叫，不重複 fetch）。**⚠️ 誠實侷限（尚未解決）**：具名提及抽取（NER）不在任何既有模組職責範圍內，是尚待補齊的上游依賴——`prepare_svo_ready_chunks()` 目前唯一可行的呼叫方式是 `mentions=None`，此時跳過整個 §a 別名登記表階段，只完成代名詞消解，未完成別名收斂，這是系統目前的真實狀態，非刻意簡化。
>
