...
flowchart TD
    ...
    SENTEMBED --> SVOGROUP["SVO 專用切塊：依序聚合句子，<br/>最多 5 句或最多 300 字元（先到者為準）<br/>🟡 工程預設，不沿用 3.1.1 的 500 字元 RAG 切塊<br/>⚠️ 是否需要重疊尚未定案"]
```
...
>
> ✅ **`SVOGROUP` 組塊上限（2026-07-21 定案並實作，數值仍待第五章校準）**：先採簡單固定聚合（依序聚合句子，最多 5 句或最多 300 字元，先到者為準），語意斷點式分組（07 報告的核心演算法）因文獻支撐不足暫緩（見 `03_變更紀錄.md` 與 `06_SVO抽取管線調整任務書.md` 第 5 節的查證過程，07 報告已據此訂正、降級為第五章候選對照方案）。300 字元這個數字沒有直接文獻依據——GraphRAG（Edge et al., 2024）附錄查證到的方向性發現是「切塊越小、抽取密度越高」，其測試範圍是 600/1200/2400 **token**（非中文字元，換算不精確，僅供粗略參考），比 3.1.1 現行 500 字元的 RAG 切塊更小的方向站得住腳，但具體數值本身是工程預設，非嚴謹推導，留給第五章消融實驗校準。**實作**：`services/svo_chunking.py::build_svo_chunks()` 的 `max_sentences`（預設 5）／`max_chars`（預設 300）參數，任一先達到上限即切分，測試見 `tests/services/test_svo_chunking.py`。
