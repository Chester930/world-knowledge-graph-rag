...
>
> **尚待決策清單（依討論順序）**：
> 1. 滑動視窗大小 `window_size` 與重疊句數 `overlap` 的具體數值——是否需要文獻依據，還是先取經驗值、留給第五章消融實驗校準（比照 `CLASSIFY_AUTO_THRESHOLD` 現行做法）？
