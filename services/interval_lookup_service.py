...
不降反升——判斷失效根因被判定為「這類查表本質上不該交給弱 LLM judge」。這個結論
的**直接證據是本專案自己的金絲雀驗證**；RefusalBench（Muhamed et al., 2025）只是
間接參考：它在單文件 NQ 基準上報告 Qwen 各尺寸 selective-refusal 準確率 <17%，但量的
是模型在脈絡有缺陷時是否適當拒答（生成行為），不是查表判斷（準確範圍見報告61 §3.1）。
