...
def _grounding_prompt(
    ...
    *,
    ...
    # 報告32 §9 G2 方案E（2026-09-13）：「數值區間 → 對應值」查表判斷改由
    # `services/interval_lookup_service.py` 的確定性規則覆核（`chat()` 呼叫端），
    # 不再靠這裡的 prompt 指令讓 judge 自己判斷——C 驗證（`run_refusal_canary.py`）
    # 證實這類判斷交給 qwen judge 不可靠（MRR 上升）。此 prompt 保持嚴格：查表
    # 例外不在這裡處理，一律走下方 false 分支，交給呼叫端的確定性覆核修正。
...
- false：這句主張的具體內容在已知事實清單裡找不到依據——即使主題相關、即使是合理推論，只要具體數字或結論沒有在清單裡逐字出現過，就算 false，不可因為「聽起來合理」就判定 true（含「數值區間 → 對應值」分段對照表的查表推論——這類判斷改由呼叫端的確定性規則覆核處理，這裡一律先判 false）。
...
async def verify_fact_grounding(
    ...
    *,
    ...
    `question`（keyword-only，2026-09-08 報告32 §9 A′；2026-09-13 方案E後
    G2 查表判斷已移出本函式，此參數保留供未來其他核對規則使用，目前不影響
    輸出）：未傳入時 prompt 略去「使用者問題」區塊，其餘行為與舊版逐字相同。
    """
