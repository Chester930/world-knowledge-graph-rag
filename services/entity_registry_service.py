...
**2026-07-21 再修訂（PK 主規則依範圍而異，使用者提出）**：單一文件內，實體
通常是「先給完整名稱、後續才為精簡改用簡稱/代名詞式簡稱」——若沿用頻率優先，
幾乎必然選到「後面反覆使用的簡稱」為標準名，反而犧牲了完整性，也讓下游代名詞
消解的實體接力上下文變得不夠清楚。因此**本模組（§a，文件內範圍）改採長度優先
為主規則**（`should_promote_by_length()`），出現次數僅作平手時的次要規則；
`services/svo_service.py`（§b，跨文件範圍）**維持頻率優先為主規則**
（`should_promote_by_frequency()`，原 `should_promote()` 更名）不變——這樣
劃分也讓頻率優先規則更精準對應其文獻依據（Wikidata「社群共識最常用名稱」、
CESI「頻率加權質心」本來就是跨文件/跨編者尺度的頻率概念，套用在單一文件內
本屬勉強延伸）；長度優先本身仍查無文獻先例，維持標註為本論文自行設計。
"""
...
def should_promote_by_length(candidate_count: int, current_count: int, candidate: str, current: str) -> bool:
    """§a 文件內範圍的 PK 規則（2026-07-21 修訂）：字面長度優先；長度相同
    才比文件內出現次數；皆相同則保留既有 Key（回傳 False）。長度優先本身
    查無文獻先例，屬本論文自行設計的工程決策（見模組 docstring）。"""
    if len(candidate) != len(current):
        return len(candidate) > len(current)
    return candidate_count > current_count


def should_promote_by_frequency(candidate_count: int, current_count: int, candidate: str, current: str) -> bool:
    """§b 跨文件範圍的 PK 規則（09 報告 § 3.2，2026-07-21 修訂；原名
    `should_promote()`）：出現頻率優先；頻率相同才比字面長度；長度也相同
    則保留既有 Key（回傳 False）。呼叫端（`services/svo_service.py`）的
    `candidate_count`／`current_count` 應為跨文件計數（獨立文件數，而非
    chunk/提及次數），見該模組 `_aggregate_alias_counts()`。"""
...
class EntityRegistry:
    ...
    async def resolve_mention(
        ...
        → PROMOTE（出現次數 +1 後與現有 Key 比較，長度優先，次數次規則）→ REPLACE。
        """
        mention = _normalize(mention)
        if not mention:
        ...
        if should_promote_by_length(candidate_count, current_count, mention, key):
